...
| src/main/resources/META-INF/spring/spring-security.xmlを設定する。
...
* spring-mvc.xml
  ...
  .. note::
       use-expressions="true" とすることで有効になるアクセス属性の指定方法については、
       \ `Expression-Based Access Control <http://static.springsource.org/spring-security/site/docs/3.1.x/reference/el-access.html>`_\ を参照されたい。

Web.xmlの設定
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

| 上記で用意したspring-security.xmlを、web.xmlで指定する。
