...
How to exclude a specific field indicated below.
...
.. tip::
    ...
    Instead of specifying the exclude field, it is also possible to specify to use only specific fields.
     ...
     .. list-table::
        ...
        * - Sr.No
          - Description
        * - | (2)
          - In case of use only specific fields specify the  name of fields that you want to include into \ ``of`` \ attribute of \ ``@EqualsAndHashCode`` \ annotation.

            In the above example, \ ``equals`` \ method and \ ``hashCode``\ method gets generated by referring only \ ``itemCode`` \ field.
