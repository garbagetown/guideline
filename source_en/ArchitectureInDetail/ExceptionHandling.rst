...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The exceptions that occur when running an application are classified into 3 types shown below.

.. list-table:: **Table – Types of exceptions occur when running an application**
   :header-rows: 1
   :widths: 10 30 30 30
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
| The exceptions which occur when running an application, are handled using following 4 methods.
...
.. list-table:: **Table - Exception Handling Methods**
   ...
   * - | (3)
     ...
     - | Use this in order to handle exceptions at servlet level.
       | When \ ``<mvc:annotation-driven>``\  is specified, HandlerExceptionResolver automatically uses \ :ref:`Class to be registered<ExceptionHandling-annotation-driven>`\ , and \ ``SystemExceptionResolver``\  provided by this framework.
...
.. note:: **Where to use @ControllerAdvice annotation?**
  ...
  #. In case of exception handling carried out at servlet level, when model (JavaBeans) for error is to be generated by serializing in JSON or XML formats
     without using template engines such as JSP for error response data
     (Used as error handling for creating Controller for AJAX or REST).
...
Types of exceptions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The exceptions that occur when running an application are classified into the following 6 types.
...
| **Exception to notify a violation of a business rule**
| This exception is generated in the logic of the domain layer.
| These states are assumed in the application so need not be dealt by the system administrator.
...
.. note:: **Corresponding Exception Class**
  ...
  - If the requirements cannot be met by the business exception class provided by common library, a business exception class should be created for each project.


.. _exception-handling-exception-type-libraryexception-label:

Library exception that occurs during normal operation
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
.. todo::
  ...
  *If pessimistic locking fails, the child class of \ ``org.springframework.dao.UncategorizedDataAccessException``\  occurs and not \ ``PessimisticLockingFailureException``\ .
  ...
  This exception can be handled since exception notifying that pessimistic locking error has occurred, has been saved as cause of exception.
...
| This exception should be generated in the logic of application layer and domain layer.
...
.. note:: **Corresponding Exception Class**
  ...
  - If the requirements for the system exception class provided in the common library are not met, a system exception class should be created for each project.
...
| **Fatal Errors (error object that inherits java.lang.Error) must not be handled (try-catch) in the application code. **
...
.. note:: **Example of corresponding Exception Class**
  ...
  - \ ``org.springframework.web.HttpRequestMethodNotSupportedException``\  (Exception that occurs when the access is made through a method which is not supported).
  - \ ``org.springframework.beans.TypeMismatchException``\  (Exception that occurs if a value that cannot be converted to URI is specified).
  - etc ...

 \ A class corresponding to exception where HTTP status code is "4XX" in \ :ref:`exception-handling-appendix-defaulthandlerexceptionresolver-label`\ .
...
| (1)-(2) should be handled at use case level and (3)-(6) should be handled in the entire system (application).
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
When partial redo (from middle) of a use case is notified, catch (try-catch) the exception in the application code of Controller class and carry out exception handling at request level.
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
When redo of a use case (from beginning) is notified, catch the exception using \ ``@ExceptionHandler``\ , and carry out exception handling at use case level.

.. note:: **Example when notifying redo of a use case (from beginning)**

  - | If the product master to be changed on shopping site (site for administrator), has been changed by other operator (in case of optimistic locking exception)
...
.. note:: **Examples for notifying that the system or application is not in a normal state**
  ...
  - | In case of a use case for connecting to an external system, if an exception notifying that the external system is blocked occurs
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
When a fatal has been detected, catch the exception using servlet container and carry out exception handling at web application level.
...
| **The description of processing to be implemented in the application code is indicated in Bold.**
| The log of stack trace and exception messages is output in classes (Filter and Interceptor class) provided by the common library.
| When log of information other than exception messages and stack trace needs to be output, it should be implemented separately in each of the logic.
| This section describes the flow of exception handling; hence the description related to flow till the Service class is called is omitted.
...
#. **Controller class catches BusinessException and sets message information (ResultMessage) set in the BusinessException to the Model for screen display(6').**
...
Example of implementing the above flow is given below.
...
| Refer to the figure below.
| It illustrates the basic flow at the time of handling the business exception (\ ``org.terasoluna.gfw.common.exception.SystemException``\ ) provided by this framework using \ ``org.terasoluna.gfw.web.exception.SystemExceptionResolver``\ .
...
#. The response generated by the View called by the Controller, is displayed.
...
How to use
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The usage of exception handling function is described below.
...
| A blank project has already been set, hence \ **[Locations to be customized for each project] **\  can be modified.
...
(1) Add logger class (\ ``ExceptionLogger``\ ) outputting exception log, to bean definition.
 ...
 .. list-table::
    ...
    * - | (2)
      - | Specify mapping between name of the exception to be handled and applicable "Exception Code (Message ID)".
        ...
        | **[Locations to be customized for each project]**
    * - | (3)
      - | Specify default "Exception Code (Message ID)".
        ...
        .. note:: **Exception Code (Message ID)**
             ...
             It is also possible to create an ID which need not be in the format defined in Properties but can be identified in the application.
             For example, MA7001 etc.

    * - | (4)
      - | Add \ ``ExceptionLogger``\  to bean definition.
    * - | (5)
      - | Use dependency injection for \ ``ExceptionCodeResolver``\ .
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
When exceptions (BusinessException,ResourceNotFoundException) holding ResultMessages occur, add AOP settings and interceptor class (\ ``ResultMessagesLoggingInterceptor``\ ) for log output, to bean definition.
...
.. _exception-handling-how-to-use-service-pointcut-aop-label:
 ...
 .. list-table::
    ...
    * - Sr. No.
      - Description
    * - | (1)
      - | Add \ ``ResultMessagesLoggingInterceptor``\  to bean definition.
    * - | (2)
      - | Use dependency injection for the logger object which outputs exception log. Specify "exceptionLogger" defined in \ ``applicationContext.xml``\ .
    * - | (3)
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
When <mvc:annotation-driven> is specified, a class (\ ``SystemExceptionResolver``\ ) used for handling the exceptions which are not handled by HandlerExceptionResolver registered automatically should be added to bean definition.
...
- **spring-mvc.xml**
 ...
 .. list-table::
    ...
    * - | (2)
      - | Use dependency injection for the object that resolves exception code (Message ID). Specify \ "exceptionCodeResolver"\ defined in \ ``applicationContext.xml``\ .
    * - | (3)
      - | Specify the order of priority for handling. The value can be "3". When \ ``<mvc:annotation-driven>``\  is specified, automatically \ :ref:`registered class <exception-handling-annotation-driven>`\ is given higher priority.
        ...
        .. hint:: **Method to disable exception handling carried out by DefaultHandlerExceptionResolver**
            ...
            When exception handling is carried out by \ ``DefaultHandlerExceptionResolver``\ , HTTP response code is set; however since View is not resolved, it needs to be resolved on Error Page of \ ``web.xml``\ .
            When View is to be resolved by \ ``HandlerExceptionResolver``\  and not by \ ``web.xml``\ , if the priority order of \ ``SystemExceptionResolver``\  is set to "1", the handling process can be executed before \ ``DefaultHandlerExceptionResolver``\ .
            For the mapping of HTTP response code when handling is done by \ ``DefaultHandlerExceptionResolver``\ , refer to \ :ref:`exception-handling-appendix-defaulthandlerexceptionresolver-label`\ .
...
- **spring-mvc.xml**
 ...
 .. list-table::
    ...
    * - | (2)
      - | Use dependency injection for the logger object which outputs exception log. Specify the "exceptionLogger" defined in \ ``applicationContext.xml``\ .
    * - | (3)
...
- **web.xml**
 ...
 .. list-table::
    ...
    * - | (4)
      - | Specify the URL pattern to apply filter. It is recommended that you use \ ``/*``\  for outputting log of fatal errors and exceptions that are out of the boundary of Spring MVC.
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Add Error Page definition of Servlet Container in order to handle error response (HttpServletResponse#sendError) carried out by default exception handling function of Spring MVC, fatal errors and the exceptions that are out of the boundary of Spring MVC.

- **web.xml**

Add definitions in order to handle error response (HttpServletResponse#sendError) carried out by default exception handling function of Spring MVC.
...
.. note:: **About the path specified in location tag**
  ...
  If a fatal error occurs on specifying the path of dynamic contents, possibility of getting another error is high; hence
  in location tag, **it is recommended that you specify a path to static contents such as HTML** and not dynamic contents such as JSP.

.. note:: **If untraceable error occur during development**

  If an unexpected error response (HttpServletResponse#sendError) occurs after carrying out the above settings, there may be cases wherein the error response cannot be identified.
  ...
  When it is necessary to individually handle the exceptions that are out of the boundary of Spring MVC, definition of each exception should be added.
    ...
    .. list-table::
      ...
      * - | (5)
        - | Specify the file name. Specify it using a path from web application root. In the above settings, "${WebAppRoot}/WEB-INF/views/common/error/systemError.jsp" will be the View file.
          | **[Locations to be customized for each project]**
...
.. note:: **Notes related to the method of generating business exception**
  ...
  - | When it is required by the API specification of underlying base framework or existing layer of application to notify the violation of business logic through an exception, then it is OK to catch the exception and throw it in a business exception.
...
.. warning::
  ...
  - It is assumed that business exception is generated in Service by default. In \ :ref:`AOP settings <exception-handling-how-to-use-service-pointcut-aop-label>`\ ,
    ...
    Business exceptions should not be logged in Controller etc. It should be changed if there is a need in the project.
...
- xxxService.java
 ...
 .. list-table::
    ...
    * - | (4)
      - | Specify ResultMessages to generate ResultMessages.


.. tip::

  For the purpose of description, ``xxxService.java`` logic is written in steps (2)-(4) as shown above, however it can also be written in a single step.
...
- xxx.properties
  ...
  Describe the Property settings as reference.
...
.. note::
  ...
  This is because there is no need to inform the details of error to the development team and it is sufficient to only convey that the system has error.
  Therefore, the development side tries to enhance the response for inquiry against system errors by displaying the Message ID which acts as a key for the message text, on the screen in order to make the analysis easier.
  The displayed screens should be provided in accordance with the UI standards in each project.
...
| In this example, although fetching of customer history fails, business process does not have to be stopped and hence the execution continues.
 ...
 .. list-table::
    ...
    * - Sr. No.
      - Description
    * - | (1)
      - | Use dependency injection for the object to output log of \ ``org.terasoluna.gfw.common.exception.ExceptionLogger``\  provided by the common library.
    * - | (2)
      - | Catch the exception to be handled.
    * - | (3)
      - | Output the handled exception to log. In the example, log method is being called; however the output level has been determined,
        | so if there is no possibility of any change, it is ok to call info, warn, error methods directly.
    * - | (4)
...
An application log as shown below is output.
 ...
 .. warning::
   ...
   When log() is used in \ ``exceptionLogger``\ , since it will be output at error level; it will also be output in monitoring log by default.
 ...
 | As shown in this example, if there is no problem in continuing the execution, and if monitoring log is being monitored through application monitoring, it should be set to a level such that it will not be monitored at output level or defined such that it is not monitored from message.
 ...
 | As per default settings, only error level log is output as monitoring log. Application log is output as follows:
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
| Handle the exception at request level and set the transfer information (message information) to Model.
| Then, by calling the method for displaying the View, generate the model required by the View and determine the View name.
 ...
 .. list-table::
    ...
    * - Sr. No.
      - Description
    * - | (1)
      - | Define a Model in argument as an object to link the error information with View.
    * - | (2)
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
| Handle the exception at use case level and generate ModelMap (ExtendedModelMap) wherein transfer information (message information etc.) is stored.
| Then, by calling the method for displaying the View, generate the model required by the View and determine the View name.
...
- Output Screen (Without exceptionCode)
 ...
 .. note:: **Exception code (message ID)**
    ...
    - By outputting the exception code (message ID), it is possible to respond quickly to the inquiries from system user.
...
.. list-table:: **Table - Classes under org.terasoluna.gfw.common.exception package**
   ...
   * - | (7)
     - | ResultMessages
       | LoggingInterceptor
     - | This is an interceptor class for logging an occurrence of the exception storing \ ``ResultMessages``\  (sub exception of \ ``ResultMessagesNotificationException``\ ).
   ...
   * - | (8)
     - | BusinessException
     - | This is an exception class to notify a violation of business rules. This exception should be generated in the logic of domain layer.
       | It inherits \ ``java.lang.RumtimeException``\ ; hence the transactions are rolled back as per default behavior.
   ...
   * - | (12)
     ...
     - | This is an interface indicating that it has a role to hold the exception code and is implemented by \ ``SystemException``\  in the common library.
       | If an exception class that implements this interface is created, it can be used as is with the exception code held in the exception, in the exception handling process provided by the common library.
...
.. list-table:: **Table - Classes under org.terasoluna.gfw.web.exception package**
   ...
   * - | (14)
     ...
     - | This is a class for handling the exceptions that will not be handled by \ ``HandlerExceptionResolver``\ , which is registered automatically when \ ``<mvc:annotation-driven>``\  is specified.
       | It inherits \ ``SimpleMappingExceptionResolver``\  provided by Spring MVC and adds functions so that ResultMessages of exception code can be referenced from View.
   * - | (15)
     - | HandlerException
       | ResolverLogging
       | Interceptor
...
.. list-table:: **List of settings not explained above**
   ...
   * - | (1)
     ...
     - | Specify the attribute name (String), used for setting the information related to message of business exception in the model.
       | It is an attribute name to be used for accessing the business exception message from View(JSP).
     - resultMessages
   * - | (2)
     ...
     - | Specify the attribute name (String), used for setting the exception code (message ID) to HttpServletRequest.
       | It is an attribute name to be used for accessing the exception code (message ID) from View(JSP).
     - exceptionCode
   * - | (3)
   ...
   * - | (4)
     ...
     - | exceptionAttribute
     - | Specify the attribute name (String), used for setting the handled exception object to the model.
       | It is an attribute name to be used for accessing the exception object from View(JSP).
     - exception
   * - | (5)
